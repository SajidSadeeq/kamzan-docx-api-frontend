          ...
          <form action="#" class="form-validate" novalidate="novalidate" @submit.prevent="editAisle">
            <div class="nk-block nk-block-lg">
                        ...
                        <div class="col-md-6">
                          <div class="col-md-10">
                            <div class="form-group">
                              <label class="form-label" for="name">Level ID</label>
                              ...
                              <span v-if="from_errors" class="text-danger">{{ from_errors[0] }}</span>
                            </div>
                          </div>
                        </div>
                      </div>
              ...
              <div class="mt-2 text-right">
                <div class="form-group">
                  <NuxtLink to="/aisle" class="btn btn-danger d-md-inline-flex">
...
export default {
  ...
  computed: {
    level () {
      return this.$store.state.level.edit_level
    }
  },
  created () {
    if (Object.keys(this.level).length === 0) {
      this.$store.dispatch('level/fetchSpecificLevel', this.$route.params.levelId)
    }
  },
  methods: {
    editAisle () {
      const self = this
      this.$axios.put(`level/${this.level.id}`, {
        name: this.level.name
      }).then(function (response) {
        if (response.data.status !== false) {
          self.$router.push('/level')
        } else {
          self.from_errors = response.data.payload.error
        }
      }).catch(function (exp) {
        self.from_errors = exp.data.payload.error
      })
    }
